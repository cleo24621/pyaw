...
class SCMULF:
    def __init__(self, fp):
        ...
        self.datetimes = pd.to_datetime(verse_time, origin='2009-01-01', unit='ms')
    ...
    def concat_data(self):
        ...
        df1c = pd.DataFrame(index=self.datetimes.values, data=dict1c)  # todo: index name
        return df1c
...
class EFDSCMClip:
    def __init__(self, st, et, fp_efd, fp_scm, threshold: float = 100):
        ...
        assert all(np.diff(
            self.scm_datetimes_clip) - b_theory_interval < 10 * pd.Timedelta(1 / self.scm.fs, unit='s')), "the datetime duration of clipped b minus the theory datetime duration should be less than the set threshold."
        assert all(np.diff(
            self.efd_datetimes_clip) - e_theory_interval < pd.Timedelta(1 / self.efd.fs, unit='s')), "the datetime duration of clipped e minus the theory datetime duration should be less than the set threshold."
        self.efd_geo = pd.DataFrame(index=self.efd.datetimes.values,
        ...
        self.A111_W = pd.DataFrame(index=self.efd.datetimes.values, data=self.efd.dfs['A111_W'].values)
        self.A112_W = pd.DataFrame(index=self.efd.datetimes.values, data=self.efd.dfs['A112_W'].values)
        self.A113_W = pd.DataFrame(index=self.efd.datetimes.values, data=self.efd.dfs['A113_W'].values)
        self.scm_geo = pd.DataFrame(index=self.scm.datetimes.values,
        ...
        self.A231_W = pd.DataFrame(index=self.scm.datetimes.values, data=self.scm.dfs['A231_W'].values)
        self.A232_W = pd.DataFrame(index=self.scm.datetimes.values, data=self.scm.dfs['A232_W'].values)
        self.A233_W = pd.DataFrame(index=self.scm.datetimes.values, data=self.scm.dfs['A233_W'].values)
    ...
    def b_resampled(self):
        ...
        butters = [utils.Butter(i.values.flatten(), fs=self.scm.fs) for i in
                     [self.b_enu1, self.b_enu2, self.b_enu3]]
        b_filtered_ls = [i.apply_lowpass_filter(lowcut=self.lowcut,order=5) for i in butters]
        b_resample_ls = []
    ...
    def preprocess_data(self):
        ...
        for (column_name, column_data), (e0_col, e1_col) in zip(self.data[['e_enu1', 'e_enu2', 'e_enu3']].items(),
            ...
            _ = utils.move_average(column_data, self.target_fs * 20)
            data_dict[e0_col] = _
