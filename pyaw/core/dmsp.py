...
from pyaw.configs import DMSPConfigs
import xarray as xr
...
class SPDF:
    class SSIES3:
        def __init__(self, file_path):
            ...
            self.df = self._quality_process()  # DataFrame after quality process
        ...
        def _other_process(self):
            df = self._quality_process()  # （没有使用.copy()）质量控制后返回的DataFrame的视图
            # rename
    ...
    class SSM:
        ...
        def _process(self):
            ...
            """
            df = self.original_df.copy()  # because `data_.columns = data_.columns.str.lower()` will change the input data's columns names, for not confused, use `copy`.
            ...
            df.rename(columns={'delta_b_sc1': 'b1_ssm_sc1', 'delta_b_sc2': 'b1_ssm_sc2', 'delta_b_sc3': 'b1_ssm_sc3',
    ...
    class SSIES3CoupleSSM:
        ...
        #
        # def ssm_sc2s3_sc(self, com1: pd.Series, com2: pd.Series, com3: pd.Series) -> tuple[
        ...
        #     return pd.DataFrame(np.cross(v.values, B.values) * 1e-6 * -1, columns=['1', '2', '3'],
