...
mea_B_data_north, mea_B_data_east,_ = other.get_3arrs(data_mea['B_NEC'].values)
igrf_B_data_north,igrf_B_data_east,_ = other.get_3arrs(data_igrf['B_NEC_IGRF'].values)
delta_B_data_north,delta_B_data_east = (mea_B_data_north - igrf_B_data_north, mea_B_data_east - igrf_B_data_east)
...
# 插值
delta_B_grid = griddata((mlat_data, mlon_data), delta_B_data, (MLAT, MLON), method='linear')
...
pcm = ax.pcolormesh(MLON, MLAT, delta_B_grid, transform=ccrs.PlateCarree(), cmap='RdBu_r')
cbar = plt.colorbar(pcm, ax=ax, orientation='vertical', label='扰动磁场 (nT)')

ax.plot(mlon_data, mlat_data, color='black', linewidth=0.5, transform=ccrs.PlateCarree())
